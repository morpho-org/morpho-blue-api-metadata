...
- `chainId`: Chain ID where the token is deployed (supports 1, 8453, 10, 130, 137, 143, 480, 988, 999, 4217, 42161, 747474, 98866)
...
The custom warnings file is an array containing both vault and market warnings. The type of warning is determined by the presence of either `vaultAddress` (for vaults) or `marketId` (for markets).
...
**For Market Warnings:**
- `marketId`: 32-byte hex string (0x + 64 hex characters) identifying the market
...
3. Market IDs must be 32-byte hex strings (66 characters total: 0x + 64 hex chars)
4. No duplicate vaultAddress + chainId combinations
5. No duplicate marketId + chainId combinations
